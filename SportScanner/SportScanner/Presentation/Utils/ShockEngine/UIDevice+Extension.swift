...
extension UIDevice {
    ...
    func statusBarHeight() ->CGFloat {
        ...
        var height = UIApplication.shared.windows.filter {$0.isKeyWindow}.first?.windowScene?.statusBarManager?.statusBarFrame.height ?? 0
        ...
        if deviceType == DeviceType.iPhone11 {
            height = 48
        } else if ((deviceType == DeviceType.iPhone12) || (deviceType == DeviceType.iPhone12Pro) || (deviceType == DeviceType.iPhone12ProMax)) {
            height = 47
        } else if (isiPhoneXSerie()) {
            height = 44
        } else {
            height = 20
        }
    ...
    func isiPhoneXSerie () -> Bool {
        ...
        if mainWindow??.safeAreaInsets.bottom ?? 0 > 0.0 {
